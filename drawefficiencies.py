...
with open('combined.csv', 'r') as file:
    ...
    i = 0
    for row in reader:
        stubsplot.SetPoint(i,float(row[0]),float(row[4]))
        pixelplot.SetPoint(i,float(row[0]),float(row[2]))
        realstripplot.SetPoint(i,float(row[1]),float(row[3]))
        i += 1
...
with open('combined.csv', 'r') as file2:
    reader2 = csv.reader(file2)
    i = 0
    for row in reader2:
        stripplot.SetPoint(i,((float(row[1])) - stripxmin) * stubrange / striprange + stubsxmin,float(row[3]))
        i +=1
...
canvas.Update()
canvas.SaveAs("ThScanPlot.png")
print("The plot for the non-irradiated threshold scan has been drawn in ThScanPlot.png")
